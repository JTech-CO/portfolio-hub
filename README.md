...
3. `python tools/validate_catalog.py`로 데이터를 검증합니다.
4. 브라우저를 새로고침합니다.
...
`id`, `name`, `shortDescription`만 필수입니다. 이미지 아이콘은 프로젝트 폴더에 넣고 `portfolio/<category>/icon.png`처럼 지정할 수 있습니다. Font Awesome 아이콘은 `fas fa-cube` 형식으로 지정합니다. 링크가 아직 없다면 `url`을 `null`로 두면 모달 버튼이 자동으로 비활성화됩니다.

상태값은 `active`, `beta`, `coming-soon` 중 하나를 사용합니다. 각 `items.json`은 `portfolio/items.schema.json`을 참조하므로 지원하는 편집기에서 자동 완성과 오류 표시를 받을 수 있습니다.
...
2. `portfolio/categories.json`에 `key`, `label`, `source`를 추가합니다.
...
js/catalog.js                   JSON 로딩, 정규화, 오류 격리
js/card.js                      카드 DOM 생성
